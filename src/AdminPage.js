...
// Helper function to check if URL needs imgbb upload
function needsImgbbUpload(url) {
  if (!url) return false;
  
  const redirectUrls = (process.env.REACT_APP_IMGBB_REDIRECT_URLS)
    .split(',')
...
function AdminPanel() {
  ...
  // Modified saveShow to handle flyer URLs that need imgbb upload
  const saveShow = useCallback(async (e) => {
    ...
    try {
    // Check if flyer URL needs to be uploaded to imgbb
    if (flyer && needsImgbbUpload(flyer)) {
